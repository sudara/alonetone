class PurgeEligibleRecordsJob < ApplicationJob
  ...
  def perform(users: true, assets: true, comments: true, limit: nil, dry_run: false)
    ...
    purged_users = users ? User.destroy_deleted_older_than_30_days(limit: limit, dry_run: dry_run) : 0
    purged_assets = assets ? Asset.destroy_deleted_older_than_30_days(limit: limit, dry_run: dry_run) : 0
    purged_comments = comments ? Comment.destroy_deleted_older_than_30_days(limit: limit, dry_run: dry_run) : 0
