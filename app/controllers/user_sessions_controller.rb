class UserSessionsController < ApplicationController
  ...
  def create
    ...
    if @user_session.save
      ...
      flash.now[:error] = if params[:user_session][:login] && (user = User.find_by_login(params[:user_session][:login])) && !user.active?
                            "It looks like your account is not active. <br/> Do you have an email from us with activation details?".html_safe
                          else
                            "There was a problem logging you in! Please check your login and password."
                          end
      @user = User.new
      @bypass_recaptcha = true unless RECAPTCHA_ENABLED
      render action: :new
