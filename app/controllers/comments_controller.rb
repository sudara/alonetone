class CommentsController < ApplicationController
  ...
  def index
    if params[:login].present?
      ...
      @pagy, @comments = pagy(@user.comments.on_track.public_or_private(display_private_comments?).includes(commenter: :pic, commentable: { user: :pic }))
      set_comments_made
    else
  ...
  def set_comments_made
    @pagy_comments_made, @comments_made = pagy(Comment.where(commenter_id: @user.id).public_or_private(display_private_comments?))
  end

  def set_spam_comments
    @pagy_spam, @spam = pagy(Comment.spam) if moderator?
  end
