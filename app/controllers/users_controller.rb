class UsersController < ApplicationController
  ...
  def gather_user_goodies
    ...
    @favorites = @user.tracks.favorites.without_deleted_assets.includes(asset: { user: :pic }).limit(5).collect(&:asset)
  ...
  def gather_assets
    return @user.assets.with_deleted.includes(user: :pic).limit(5) if current_user&.moderator?
