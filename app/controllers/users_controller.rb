class UsersController < ApplicationController
  ...
  def passed_recaptcha?
    @bypass_recaptcha = if (session[:recaptcha] == true) || !RECAPTCHA_ENABLED
                          true # bypass when already entered or setting not present
                        else
                          session[:recaptcha] = verify_recaptcha(model: @user)
                        end
  end
