...
module PreventAbuse
  ...
  # Memoized because is_a_bot? runs it 2-3x per request on the listen hot path.
  def is_from_a_bad_ip?
    return @is_from_a_bad_ip if defined?(@is_from_a_bad_ip)

    @is_from_a_bad_ip = BannedIp.banned?(request.remote_ip)
  end
